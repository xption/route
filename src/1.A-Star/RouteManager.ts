...
export class RouteManager {
    ...
    public static Route(mapData: MapData, cellFrom: Cell, cellTo: Cell): void {
        ...
        let closeSet: Set<string> = new Set<string>();
        ...
        while (openMap.size > 0) {
            ...
            closeSet.add(minFCell.getKey());

            let aroundCellArray: Cell[] = RouteManager.FindAroundCellArray(minFCell, cellTo, mapData);
            for (let i = 0; i < aroundCellArray.length; i++) {
                let aroundCell: Cell = aroundCellArray[i];
                if (closeSet.has(aroundCell.getKey())) {
                    continue;
                }
    ...
    private static FindAroundCellArray(centerCell: Cell, toCell: Cell, mapData: MapData): Cell[] {
