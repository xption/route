...
export class RouteManager {
    ...
    public static Route(mapData: MapData, cellFrom: Cell, cellTo: Cell): void {
        ...
        let openPQ: MaxPQ = new MaxPQ();
        openPQ.insert(cellFrom);

        let closeMap: Map<string, Cell> = new Map<string, Cell>();

        while (openMap.size > 0) {
            let minFCell: Cell = openPQ.delMax();
            if (minFCell == null) {
                console.log('no minFCell');
                break;
            }
            ...
            for (let i = 0; i < aroundCellArray.length; i++) {
                ...
                if (closeMap.has(aroundCell.getKey())) {
                    Pool.Set(aroundCell);
                    continue;
                }
